    ...
    try {
          ...
          return new ImageResponse(
              ...
              {hook && <div style={{ fontSize: '36px', textAlign: 'center', maxWidth: '900px', color: '#e0d0ff', display: 'flex' }}>{hook.length > 80 ? `${hook.substring(0, 80)}...` : hook}</div>}
                      <div style={{ position: 'absolute', bottom: '40px', fontSize: '28px', color: '#b0a0d0', display: 'flex' }}>타로 한 장으로 말해줘</div>
                    </div>,
      ...
      return new ImageResponse(
              <div style={{ width: '1200px', height: '630px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #1a0a2e 0%, #2d1b4e 50%, #4a2c6e 100%)', color: '#ffd700', fontSize: '72px', fontWeight: 'bold', fontFamily: 'system-ui, sans-serif' }}>🔮 너는 나한테 어떤 사람?</div>,
        { width: 1200, height: 630 }
    );
}
}
